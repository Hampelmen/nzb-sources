<script lang="ts">
    ...
    import {fmtLimit, langStore, regLabel, sReplace, strings, tApiKey, tMembership,} from '@/lib/i18n';
    ...
    function rowMatches(r: Indexer, q: string) {
        const big =
            [
                r.name,
                r.id,
    ...
    $: filtered =
        search.trim() === '' ? rows : rows.filter((r) => rowMatches(r, search.toLowerCase()));
    ...
    $: sorted = [...filtered].sort((a, b) => {
        ...
        if (sortDir === 'asc') return av.localeCompare(bv, undefined, {sensitivity: 'base'});
        return bv.localeCompare(av, undefined, {sensitivity: 'base'});
    });
...
<div class="max-h-[500px] overflow-auto rounded-lg shadow" bind:this={scrollEl}>
	<table class="table-zebra table w-full">
		...
		<tr>
			...
			{#if !disabledSet.has('payment') && visible.payment}
				<th
				>{strings[$langStore].tableHeaders.payment}</th
				>
			{/if}
			{#if !disabledSet.has('crypto') && visible.crypto}
				<th
				>{strings[$langStore].tableHeaders.crypto}</th
				>
			{/if}
			{#if !disabledSet.has('content') && visible.content}
				<th
